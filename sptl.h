...
static uint32_t hash_fnv(const char *data, const size_t bytes) {
    ...
    for (size_t i = 0; i < bytes; ++i) {
        hash = (hash ^ (uint32_t) data[i]) *
               FNV_PRIME_32; // EXPERIMENTAL: casting to uint32_t may break hash function
    }
...
#define sp_ht_get(ht, expected_key)                                                                          \
    (sp_ht_hash((ht), (expected_key)) != (ht)->capacity                                                      \
         ? ((ht)->nodes[sp_ht_hash((ht), (expected_key))].key                                                \
                ? &((ht)->nodes[sp_ht_hash((ht), (expected_key))])                                           \
                : NULL)                                                                                      \
         : NULL)
